...
发布镜像不得在开机时自动执行 `git pull`。版本升级应通过显式构建和验证完成。
...
- 仓库统一使用 GitHub 名称 `xiaozhi-esp32-server` 与 `index-tts`；运行服务仍使用“小智服务端”和“IndexTTS 2.5”。
...
- 部署分支固定为：`xiaozhi-esp32-server` 使用 `mvp`，`index-tts` 使用 `main`；不得展示或比较其他分支。
- 工作区状态与远端状态分开：本地修改仍允许检查远端，但锁定自动更新并说明具体阻塞文件。
- 安全更新目标只能是仓库配置的远端部署分支。
- 有受保护的本地改动、无法快进或依赖清单变化时拒绝自动更新。
