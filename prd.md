...
- GitHub 网络策略默认为“自动选择”：Git 子进程先直连 5 秒，失败后回退到 `/etc/network_turbo` 提供的 AutoDL 学术加速；也支持仅直连、仅学术加速和自定义 HTTP/SOCKS5 代理。代理不得写入全局环境，不得影响模型、语音、Java 或其他业务服务。
- 检查中显示当前连接/回退/获取更新阶段、实际线路和已等待时间；失败时区分超时、DNS、TLS、连接与 HTTP 错误，保留具体原因并提供“网络设置”和“重新检查”。
...
- 三个仓库使用相同的项目专属步骤、实时进度、耗时与日志视图；成功结果自动折叠为摘要，失败结果保持展开，数据盘保留最近 10 次更新历史。
